...
#' The plot shows that we're modeling different nonlinearities in the forest
#' habitat for low, medium and high elevations. Effectively we are crudely
#' modeling the interaction between habitat, latitude, and elevation. Since our
#' goal is prediction we'd be most interested in plotting predictions within
#' some region to make a map of predicted species richness for the area from
#' which the data came. For that we'd need maps of the predictor variables but
#' such a visualization scales to any number of predictor variables.
#' 
...
#' randomness of the folds. LOOCV does not change because the tree model and
#' training algorithms are deterministic. As before, we'll need repeated folds
#' for a more stable estimate of the 5-fold CV:
